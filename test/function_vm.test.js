...
test('function_vm node', async(t) => {
    ...
    */
    ...
    function testSendCloning(args,done) {
        var flow = [{id:"n1",type:"function_vm",wires:[["n2"],["n2"]],func:"node.send("+args+"); msg.payload = 'changed';"},
        {id:"n2", type:"helper"}];
        helper.load(node, flow, function() {
            var n1 = helper.getNode("n1");
            var n2 = helper.getNode("n2");
    ...
    function testNonObjectMessage(functionText,done) {
        var flow = [{id:"n1",type:"function_vm",wires:[["n2"]],func:functionText},
        {id:"n2", type:"helper"}];
        helper.load(node, flow, function() {
            var n1 = helper.getNode("n1");
            var n2 = helper.getNode("n2");
            ...
            setTimeout(function() {
                try {
                    ...
                    var logEvents = helper.log().args.filter(function(evt) {
                        return evt[0].type == "function_vm";
                    });
                    logEvents.should.have.length(1);
                    var msg = logEvents[0][0];
                    msg.should.have.property('level', helper.log().ERROR);
                    msg.should.have.property('id', 'n1');
                    msg.should.have.property('type', 'function_vm');
